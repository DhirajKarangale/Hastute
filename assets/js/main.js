...
(function() {
  ...
  const portfolioLightbox4 = GLightbox({
    selector: '.portfolio-lightbox4'
  });
  const portfolioLightbox5 = GLightbox({
    selector: '.portfolio-lightbox5'
  });
  const portfolioLightbox6 = GLightbox({
    selector: '.portfolio-lightbox6'
  });
  ...
  jQuery(document).ready(function(){
    jQuery('.slider').slick({
      ...
      dots: false,
      arrows:false,
      // Slick's afterChange callback
      afterChange: function(slider, currentSlide) {
        AOS.refresh(); // Reapply AOS animations after slide change
      }
    });
